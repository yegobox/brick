...
import 'package:brick_core/core.dart'
    show ModelRepository, Provider, Query, QueryAction;
...
abstract class OfflineFirstRepository<
        TRepositoryModel extends OfflineFirstModel>
  ...
  @protected
  @visibleForTesting
  final Map<Type, Map<Query?, StreamController<List<TRepositoryModel>>>>
      subscriptions = {};
  ...
  Future<bool> delete<TModel extends TRepositoryModel>(
    ...
    if (autoHydrate) {
      unawaited(hydrate<TModel>(query: query));
    }

    return rowsDeleted > 0;
  }

  Future<int> _deleteLocal<TModel extends TRepositoryModel>(
    TModel instance, {
    Query? query,
  }) async {
  ...
  Future<List<TModel>?> getAssociation<TModel extends TRepositoryModel>(
    ...
    final results = await get<TModel>(
      query: query,
      policy:
          _latestGetPolicy ?? OfflineFirstGetPolicy.awaitRemoteWhenNoneExist,
    );
    if (results.isEmpty) {
      return null;
    }
  ...
  Future<void> migrate() async {
    logger.info('🚀 [OfflineFirstRepository] migrate() started');
    final lastVersion = await sqliteProvider.lastMigrationVersion();
    logger
        .info('ℹ️ [OfflineFirstRepository] lastMigrationVersion: $lastVersion');
    final migrations = migrationManager.migrationsSince(lastVersion);
    logger.info(
      'ℹ️ [OfflineFirstRepository] migrations to run: ${migrations.length}',
    );

    await sqliteProvider.migrate(migrations);
    logger.info('✅ [OfflineFirstRepository] migrate() completed');
  }
  ...
  Future<void>
    ...
    final queriesControllers =
        (subscriptionsByQuery ?? subscriptions[TModel])?.entries;
    if (queriesControllers?.isEmpty ?? true) {
      return;
    }

    // create a copy of the controllers to avoid concurrent modification while looping
    final cachedControllers =
        List<MapEntry<Query?, StreamController<List<TRepositoryModel>>>>.from(
      queriesControllers!,
    );
    for (final queryController in cachedControllers) {
      ...
      if (query == null || memoryCacheProvider.canFind<TModel>(query)) {
        ...
        if (!controller.isClosed && (results?.isNotEmpty ?? false)) {
          controller.add(results!);
        }
      }

      final existsInSqlite =
          await sqliteProvider.exists<TModel>(query: query, repository: this);
      if (existsInSqlite) {
        final results =
            await sqliteProvider.get<TModel>(query: query, repository: this);
        if (!controller.isClosed) {
          controller.add(results);
        }
      } else if (notifyWhenEmpty) {
        if (!controller.isClosed) {
          controller.add(<TModel>[]);
        }
      }
    }
  }
  ...
  Future<TModel> upsert<TModel extends TRepositoryModel>(
    ...
    if (query?.action == null) {
      query =
          (withPolicy ?? const Query()).copyWith(action: QueryAction.upsert);
    }
    ...
    try {
      ...
      if (requireRemote) {
        instance.primaryKey =
            await _upsertLocal<TModel>(instance, query: query);
        await notifySubscriptionsWithLocalData<TModel>();
      }
    } on ClientException catch (e) {
  ...
  Future<List<TModel>> hydrate<TModel extends TRepositoryModel>({
    ...
    try {
      ...
      final modelsFromRemote =
          await remoteProvider.get<TModel>(query: query, repository: this);

      if (modelsFromRemote != null) {
        final modelsIntoSqlite =
            await storeRemoteResults<TModel>(modelsFromRemote);
        final modelsIntoMemory =
            memoryCacheProvider.hydrate<TModel>(modelsIntoSqlite);

        if (!deserializeSqlite) {
